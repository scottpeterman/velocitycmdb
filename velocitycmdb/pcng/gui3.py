#!/usr/bin/env python3
"""
SecureCollector - Enhanced PyQt6 Network Job Runner
Desktop frontend for batch SSH automation jobs with vendor-specific command support
Added menu bar with File > Open, Save, Exit functionality and state management
"""
...
from PyQt6.QtWidgets import (
    ...
    QSplitter, QTreeWidget, QTreeWidgetItem, QHeaderView, QMenuBar
)
...
class JobExecutorThread(QThread):
    """Thread for executing batch SSH jobs with enhanced cancellation"""
    ...
    def cancel(self):
        """Cancel the running job with enhanced termination"""
        self.is_cancelled = True
        if self.process:
            try:
                self.progress_update.emit("Cancelling job...")
                ...
                try:
                    ...
                    self.progress_update.emit("Job cancelled gracefully")
                except subprocess.TimeoutExpired:
                    # Force kill if graceful termination fails
                    self.process.kill()
                    self.progress_update.emit("Job force-killed")

            except Exception as e:
                self.progress_update.emit(f"Error during cancellation: {str(e)}")
    ...
    def _get_script_args_mapping(self, script_name: str) -> Dict[str, str]:
        ...
        script_args = {
            ...
            'batch_spn_concurrent.py': {
                'verbose': '--verbose',
                'dry_run': '--dry-run',
                'max_workers': '--max-processes'  # Uses --max-processes instead of --max-workers
            }
        }
    ...
    def run(self):
        """Execute the batch job with enhanced error handling and cancellation support"""
        try:
            ...
            # Add vendor-specific arguments only if the script supports them
            # batch_spn.py and similar batch scripts don't support passing additional args to the underlying SPN script
            if self.job_config.get('vendor_args'):
                # Only add vendor args for scripts that support them (none of the current batch scripts do)
                # For now, skip vendor args for batch scripts since they don't have a mechanism to pass them through
                script_supports_vendor_args = False  # batch_spn.py doesn't support additional SPN args

                if script_supports_vendor_args:
                    vendor_args = self.job_config['vendor_args'].split()
                    cmd_args.extend(vendor_args)
                    self.progress_update.emit(f"Added vendor-specific args: {self.job_config['vendor_args']}")
                else:
                    self.progress_update.emit(
                        f"Skipping vendor-specific args (not supported by {script_name}): {self.job_config['vendor_args']}")
...
class CommandTemplateManager:
    ...
    def _get_default_templates(self) -> Dict[str, Dict[str, str]]:
        """Get default command templates with vendor-specific examples"""
        return {
            "version_check": {
                "name": "Version Check",
            ...
            "arista_version": {
                "name": "Arista Version",
                "command": "show version",
                "description": "Arista device version",
                "vendor": "arista"
            },
            "palo_system_info": {
                "name": "Palo Alto System Info",
                "command": "show system info",
                "description": "Palo Alto system information",
                "vendor": "paloalto"
            },
            "cloudgenix_overview": {
                "name": "CloudGenix Overview",
                "command": "dump overview",
                "description": "CloudGenix device overview",
                "vendor": "cloudgenix"
            }
        }
    ...
    def get_templates_by_vendor(self, vendor: str) -> List[Dict[str, str]]:
        """Get templates filtered by vendor"""
        vendor_templates = []
        for template in self.templates.values():
            if isinstance(template, dict) and template.get('vendor', '').lower() == vendor.lower():
                vendor_templates.append(template)
        return vendor_templates


class NetworkJobRunner(QMainWindow):
    """Main application window with enhanced vendor support and state management"""

    def __init__(self):
        super().__init__()
        self.setWindowTitle("Enhanced Network Job Runner")
    ...
    def setup_menu_bar(self):
        ...
        new_action.setStatusTip('Start a new configuration')
        ...
        open_action.setStatusTip('Open a saved configuration')
        ...
        save_as_action.setStatusTip('Save configuration with a new name')
        ...
        clear_filters_action.setStatusTip('Clear all device filters')
        ...
        # Reload Templates action
        reload_templates_action = QAction('&Reload Templates', self)
        reload_templates_action.setStatusTip('Reload command templates from file')
        reload_templates_action.triggered.connect(self.load_templates)
        tools_menu.addAction(reload_templates_action)

        # Preview Devices action
        preview_devices_action = QAction('&Preview Devices', self)
        preview_devices_action.setShortcut('F5')
        preview_devices_action.setStatusTip('Preview devices with current filters')
        preview_devices_action.triggered.connect(self.preview_devices)
        tools_menu.addAction(preview_devices_action)

    def setup_change_tracking(self):
        """Setup change tracking for form elements to detect unsaved changes"""
        # Connect all form elements to track changes
        form_elements = [
            self.session_file_edit, self.vendor_combo, self.auto_paging_check,
            self.username_edit, self.password_edit, self.enable_password_edit,
            self.credential_system_combo, self.folder_filter, self.name_filter,
            self.vendor_filter, self.device_type_filter, self.template_combo,
            self.commands_edit, self.output_dir_edit, self.batch_script_combo,
            self.max_workers_spin, self.verbose_check, self.dry_run_check
        ]

        for element in form_elements:
            if hasattr(element, 'textChanged'):
                element.textChanged.connect(self.mark_state_changed)
            elif hasattr(element, 'currentTextChanged'):
                element.currentTextChanged.connect(self.mark_state_changed)
            elif hasattr(element, 'toggled'):
                element.toggled.connect(self.mark_state_changed)
            elif hasattr(element, 'valueChanged'):
                element.valueChanged.connect(self.mark_state_changed)

    def mark_state_changed(self):
        """Mark that the state has changed and update window title"""
        if not self.state_changed:
            self.state_changed = True
            self.update_window_title()

    def update_window_title(self):
        """Update window title to reflect current file and change status"""
        base_title = "Enhanced Network Job Runner"

        if self.current_state_file:
            filename = os.path.basename(self.current_state_file)
    ...
    def get_current_state(self) -> Dict[str, Any]:
        """Capture the current state of all form elements"""
        state = {
            'version': '1.0',
            'timestamp': datetime.now().isoformat(),
            'session_file': self.session_file_edit.text(),
            'vendor': {
                'selected': self.vendor_combo.currentText(),
                'auto_paging': self.auto_paging_check.isChecked()
            },
            'credentials': {
                'username': self.username_edit.text(),
                'credential_system': self.credential_system_combo.currentText(),
                # Note: We don't save passwords for security reasons
                'password_provided': bool(self.password_edit.text()),
                'enable_password_provided': bool(self.enable_password_edit.text())
            },
            'filters': {
                'folder': self.folder_filter.text(),
                'name': self.name_filter.text(),
                'vendor': self.vendor_filter.text(),
                'device_type': self.device_type_filter.text()
            },
            'commands': {
                'template': self.template_combo.currentText(),
                'command_text': self.commands_edit.text(),
                'output_directory': self.output_dir_edit.text()
            },
            'execution': {
                'batch_script': self.batch_script_combo.currentText(),
    ...
    def set_current_state(self, state: Dict[str, Any]):
        """Restore form elements from saved state"""
        try:
            # Temporarily disable change tracking
            self.blockSignals(True)

            # Session file
            if 'session_file' in state:
                self.session_file_edit.setText(state['session_file'])

            # Vendor settings
            if 'vendor' in state:
                vendor_data = state['vendor']
                if 'selected' in vendor_data:
                    vendor_text = vendor_data['selected']
                    index = self.vendor_combo.findText(vendor_text)
                    if index >= 0:
                        self.vendor_combo.setCurrentIndex(index)

                if 'auto_paging' in vendor_data:
                    self.auto_paging_check.setChecked(vendor_data['auto_paging'])

            # Credentials (excluding passwords for security)
            if 'credentials' in state:
                creds_data = state['credentials']
                if 'username' in creds_data:
                    self.username_edit.setText(creds_data['username'])

                if 'credential_system' in creds_data:
                    system_text = creds_data['credential_system']
                    index = self.credential_system_combo.findText(system_text)
                    if index >= 0:
                        self.credential_system_combo.setCurrentIndex(index)

            # Filters
            if 'filters' in state:
                filters_data = state['filters']
                self.folder_filter.setText(filters_data.get('folder', ''))
                self.name_filter.setText(filters_data.get('name', ''))
                self.vendor_filter.setText(filters_data.get('vendor', ''))
                self.device_type_filter.setText(filters_data.get('device_type', ''))

            # Commands
            if 'commands' in state:
                commands_data = state['commands']

                if 'template' in commands_data:
                    template_text = commands_data['template']
                    index = self.template_combo.findText(template_text)
                    if index >= 0:
                        self.template_combo.setCurrentIndex(index)

                if 'command_text' in commands_data:
                    self.commands_edit.setText(commands_data['command_text'])

                if 'output_directory' in commands_data:
                    self.output_dir_edit.setText(commands_data['output_directory'])

            # Execution settings
            if 'execution' in state:
                exec_data = state['execution']

                if 'batch_script' in exec_data:
                    script_text = exec_data['batch_script']
                    index = self.batch_script_combo.findText(script_text)
                    if index >= 0:
                        self.batch_script_combo.setCurrentIndex(index)

                if 'max_workers' in exec_data:
                    self.max_workers_spin.setValue(exec_data['max_workers'])

                if 'verbose' in exec_data:
                    self.verbose_check.setChecked(exec_data['verbose'])

                if 'dry_run' in exec_data:
                    self.dry_run_check.setChecked(exec_data['dry_run'])

            # UI state
            if 'ui_state' in state:
                ui_data = state['ui_state']
                if 'current_tab' in ui_data:
                    tab_index = ui_data['current_tab']
            ...
            # Trigger updates for dependent elements
            self.vendor_changed(self.vendor_combo.currentText())
            self.template_selected(self.template_combo.currentText())
            ...
            self.log_message("Configuration restored successfully")

        except Exception as e:
            self.log_message(f"Error restoring state: {str(e)}")
            QMessageBox.warning(self, "Restore Error",
                                f"Some settings could not be restored:\n{str(e)}")
    ...
    def new_configuration(self):
        ...
        self.state_changed = False
        self.update_window_title()
        self.log_message("Created new configuration")
    ...
    def open_configuration(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            "Job Configuration Files (*.json);;All Files (*)"
        )
        ...
        if file_path:
            try:
                ...
                self.current_state_file = file_path
                self.state_changed = False
                self.update_window_title()

                self.log_message(f"Configuration loaded from: {file_path}")

                # Auto-load session file if it exists
                session_file = self.session_file_edit.text()
                if session_file and os.path.exists(session_file):
                    try:
                        self.load_session_file()
                    except Exception as e:
                        self.log_message(f"Could not auto-load session file: {str(e)}")

            except Exception as e:
                QMessageBox.critical(self, "Open Error",
                                     f"Failed to open configuration:\n{str(e)}")
                self.log_message(f"Failed to open configuration: {str(e)}")

    def save_configuration(self) -> bool:
        """Save current configuration to file"""
        if self.current_state_file:
            return self.save_to_file(self.current_state_file)
        else:
            return self.save_configuration_as()

    def save_configuration_as(self) -> bool:
        """Save current configuration to a new file"""
        default_name = "network_job_config.json"
        if self.current_state_file:
            default_name = os.path.basename(self.current_state_file)

        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save Configuration", default_name,
            "Job Configuration Files (*.json);;All Files (*)"
        )
    ...
    def reset_to_defaults(self):
        ...
        self.session_file_edit.clear()
        self.username_edit.clear()
        self.password_edit.clear()
        self.enable_password_edit.clear()
        ...
        self.credential_system_combo.setCurrentIndex(0)
        self.template_combo.setCurrentIndex(0)
        self.batch_script_combo.setCurrentIndex(0)

        # Reset checkboxes and spinboxes
        self.auto_paging_check.setChecked(True)
        ...
        self.filter_summary_label.setText("No filtering applied")
    ...
    def closeEvent(self, event):
        ...
        if self.execution_thread and self.execution_thread.isRunning():
            reply = QMessageBox.question(
                self, "Job Running",
                "A job is currently running. Do you want to cancel it and exit?",
    ...
    def setup_job_config_tab(self):
        """Setup job configuration tab with vendor support"""
        tab = QWidget()
        ...
        vendor_group = QGroupBox("Vendor Configuration")
        vendor_layout = QGridLayout(vendor_group)

        vendor_layout.addWidget(QLabel("Device Vendor:"), 0, 0)
        self.vendor_combo = QComboBox()
        self.vendor_combo.addItems([v.title() for v in self.vendor_manager.get_vendor_list()])
        self.vendor_combo.currentTextChanged.connect(self.vendor_changed)
        vendor_layout.addWidget(self.vendor_combo, 0, 1)

        vendor_layout.addWidget(QLabel("Auto-add paging disable:"), 0, 2)
        self.auto_paging_check = QCheckBox()
        self.auto_paging_check.setChecked(True)
        self.auto_paging_check.toggled.connect(self.update_command_preview)
        vendor_layout.addWidget(self.auto_paging_check, 0, 3)

        # Vendor info label
        self.vendor_info_label = QLabel()
        self.vendor_info_label.setWordWrap(True)
        self.vendor_info_label.setStyleSheet("color: #666; font-style: italic; font-size: 10px;")
        vendor_layout.addWidget(self.vendor_info_label, 1, 0, 1, 4)
        ...
        # Command configuration with vendor-aware preview
        ...
        self.commands_edit.textChanged.connect(self.update_command_preview)
        cmd_layout.addWidget(self.commands_edit, 1, 1, 1, 2)

        # Command preview with vendor-specific paging
        cmd_layout.addWidget(QLabel("Final Command:"), 2, 0)
        self.command_preview_label = QLabel()
        self.command_preview_label.setWordWrap(True)
        self.command_preview_label.setStyleSheet(
            "background-color: #f0f0f0; color: #000000; padding: 5px; border: 1px solid #ccc; font-family: monospace;")
        cmd_layout.addWidget(self.command_preview_label, 2, 1, 1, 2)

        cmd_layout.addWidget(QLabel("Output Directory:"), 3, 0)
        self.output_dir_edit = QLineEdit()
        self.output_dir_edit.setPlaceholderText("e.g., configs, version, interfaces")
        cmd_layout.addWidget(self.output_dir_edit, 3, 1, 1, 2)
        ...
        # Control buttons with enhanced cancel
        ...
        self.force_cancel_btn.clicked.connect(self.force_cancel_job)
        self.force_cancel_btn.setEnabled(False)
        self.force_cancel_btn.setStyleSheet(
            "QPushButton { background-color: #ff5722; color: white; font-weight: bold; }")
        button_layout.addWidget(self.force_cancel_btn)
        ...
        # Force update of command preview
        self.update_command_preview()
    ...
    def vendor_changed(self, vendor_text):
        """Handle vendor selection change"""
        try:
            vendor = vendor_text.lower()
            config = self.vendor_manager.get_vendor_config(vendor)

            # Update vendor info label
            info_text = f"{config['description']}\n"
            if config['paging_disable']:
                info_text += f"Paging disable command: {config['paging_disable']}\n"
            if config['additional_args']:
                info_text += f"Additional arguments: {config['additional_args']}"

            self.vendor_info_label.setText(info_text)

            # Update command preview
            self.update_command_preview()

            # Debug logging
            self.log_message(f"Vendor changed to: {vendor_text}")

        except Exception as e:
            self.log_message(f"Error in vendor_changed: {str(e)}")
            print(f"Error in vendor_changed: {e}")

    def update_command_preview(self):
        """Update the command preview with vendor-specific paging"""
        try:
            # Check if command_preview_label exists (might be called during initialization)
            if not hasattr(self, 'command_preview_label'):
                print("DEBUG: command_preview_label does not exist yet")
                return
            ...
            print(f"DEBUG update_command_preview: commands='{commands}', vendor='{vendor}', auto_paging={auto_paging}")

            if auto_paging:
                final_command = self.vendor_manager.build_command_with_paging(vendor, commands)
                print(f"DEBUG: build_command_with_paging returned: '{final_command}'")
            else:
                final_command = commands
                print(f"DEBUG: using commands as-is: '{final_command}'")

            if final_command:
                print(f"DEBUG: Setting preview label to: '{final_command}'")
                self.command_preview_label.setText(final_command)
                # Force a repaint
                self.command_preview_label.repaint()
            else:
                print("DEBUG: Setting preview label to 'No commands entered'")
                self.command_preview_label.setText("No commands entered")
                self.command_preview_label.repaint()

            # Verify what the label actually contains
            actual_text = self.command_preview_label.text()
            print(f"DEBUG: Preview label now contains: '{actual_text}'")

        except Exception as e:
            error_msg = f"Error: {str(e)}"
            print(f"DEBUG: Exception in update_command_preview: {e}")
            if hasattr(self, 'command_preview_label'):
                self.command_preview_label.setText(error_msg)
            print(f"Error in update_command_preview: {e}")
    ...
    def get_credential_env_vars(self, username: str, password: str, enable_password: str = "") -> Dict[str, str]:
        ...
        if credential_system == "Auto-detect from script":
            # All batch scripts use CRED_* format
            if "batch_spn" in selected_script:
                # Set multiple credential IDs as fallback since we don't know which ones are used
                ...
                # For direct spn.py usage, use SSH_* format
                env_vars.update({
                    'SSH_USER': username,
        ...
        # Add enable password if provided (not used by batch scripts, but kept for compatibility)
        if enable_password:
            if credential_system == "SSH_* variables (spn.py)" or (
                    credential_system == "Auto-detect from script" and "batch_spn" not in selected_script):
    ...
    def template_selected(self, template_name):
        """Handle template selection with automatic vendor detection"""
        self.log_message(f"=== TEMPLATE SELECTION DEBUG START ===")
        self.log_message(f"Selected template name: '{template_name}'")

        if not template_name:
            self.log_message("Template name is empty, calling update_command_preview and returning")
            self.update_command_preview()
            return

        # Debug: Show available templates
        available_templates = list(self.command_manager.templates.keys())
        self.log_message(f"Available templates count: {len(available_templates)}")
        self.log_message(f"First 5 templates: {available_templates[:5]}")

        template = self.command_manager.get_template_by_name(template_name)
        self.log_message(f"Retrieved template data: {template}")

        if not template:
            self.log_message("ERROR: Template data is None")
            self.update_command_preview()
            return

        if 'command' not in template:
            self.log_message("ERROR: Template missing 'command' field")
            self.update_command_preview()
            return

        # Set the command
        command_text = template['command']
        self.log_message(f"Setting command to: '{command_text}'")
        self.commands_edit.setText(command_text)
        self.log_message(f"Command field now contains: '{self.commands_edit.text()}'")

        # Try to detect vendor from template data
        detected_vendor = None

        # Check if template has explicit vendor field
        if 'vendor' in template:
            detected_vendor = template['vendor']
            self.log_message(f"Vendor field found in template: '{detected_vendor}'")
        else:
            self.log_message("No vendor field found in template")

        # Debug: Show vendor combo contents
        self.log_message("Current vendor combo box contents:")
        for i in range(self.vendor_combo.count()):
            item_text = self.vendor_combo.itemText(i)
            is_current = (i == self.vendor_combo.currentIndex())
            self.log_message(f"  Index {i}: '{item_text}' {'(CURRENT)' if is_current else ''}")

        # Update vendor selection if detected
        if detected_vendor:
            self.log_message(f"Attempting to set vendor to: '{detected_vendor}'")

            # Find the vendor in the combo box (case-insensitive)
            vendor_found = False
            for i in range(self.vendor_combo.count()):
                combo_vendor = self.vendor_combo.itemText(i).lower()
                target_vendor = detected_vendor.lower()
                self.log_message(f"Comparing combo[{i}] '{combo_vendor}' with target '{target_vendor}'")

                if combo_vendor == target_vendor:
                    self.log_message(
                        f"MATCH FOUND! Setting vendor combo to index {i}: '{self.vendor_combo.itemText(i)}'")

                    # Store old index for comparison
                    old_index = self.vendor_combo.currentIndex()
                    ...
                    new_index = self.vendor_combo.currentIndex()
                    self.log_message(f"Vendor combo changed from index {old_index} to {new_index}")

                    # Manually trigger vendor change
                    current_vendor_text = self.vendor_combo.currentText()
                    self.log_message(f"Calling vendor_changed with: '{current_vendor_text}'")
                    self.vendor_changed(current_vendor_text)
                    vendor_found = True
                    break

            if not vendor_found:
                self.log_message(f"ERROR: Could not find vendor '{detected_vendor}' in combo box")
        else:
            self.log_message("No vendor detected from template")

        # Force update command preview
        self.log_message("Calling update_command_preview...")
        self.update_command_preview()

        # Debug: Show final state
        final_command = self.command_preview_label.text() if hasattr(self, 'command_preview_label') else 'N/A'
        self.log_message(f"Final command preview: '{final_command}'")
        self.log_message(f"=== TEMPLATE SELECTION DEBUG END ===")
    ...
    def start_job(self):
        ...
        try:
            ...
            # Get final command with vendor-specific processing
            vendor = self.vendor_combo.currentText().lower()
            base_commands = self.commands_edit.text().strip()

            if self.auto_paging_check.isChecked():
            ...
            # Add vendor-specific arguments
            vendor_args = self.vendor_manager.get_additional_args(vendor)
            if vendor_args:
                job_config['vendor_args'] = vendor_args
            ...
            self.log_message(f"Final command: {final_commands}")
    ...
    def force_cancel_job(self):
        """Force cancel the running job"""
        if self.execution_thread and self.execution_thread.isRunning():
            reply = QMessageBox.question(
                self, "Force Cancel Job",
                "This will forcefully terminate the job process. This may leave some processes running.\n\n"
            ...
            if reply == QMessageBox.StandardButton.Yes:
                try:
                    if self.execution_thread.process:
                        self.execution_thread.process.kill()
                        self.log_message("Job process force-killed")

                    self.execution_thread.terminate()
                    self.execution_thread.wait(1000)  # Wait 1 second

                    if self.execution_thread.isRunning():
                        self.log_message("Warning: Could not terminate job thread")

                    # Reset UI
                    self.start_job_btn.setEnabled(True)
                    self.cancel_job_btn.setEnabled(False)
                    self.force_cancel_btn.setEnabled(False)
                    self.status_label.setText("Job force-cancelled")
                    self.progress_bar.setRange(0, 100)
                    self.progress_bar.setValue(0)

                except Exception as e:
                    self.log_message(f"Error during force cancel: {str(e)}")
    ...
    def log_message(self, message):
        ...
        try:
            ...
            # Check if log_output widget exists yet (might be called during initialization)
            if hasattr(self, 'log_output') and self.log_output is not None:
                ...
                # Fallback to console output if UI not ready
    ...
    def load_templates(self):
        ...
        try:
            # Reload templates from file to get any external changes
            self.command_manager.templates = self.command_manager.load_templates()

            # Display the actual loaded templates in the editor
            self.template_editor.setPlainText(
                json.dumps(self.command_manager.templates, indent=2)
            )

            # Update combo box with loaded template names
            self.template_combo.clear()
            template_names = self.command_manager.get_template_names()
            self.template_combo.addItems([""] + template_names)  # Add empty option first

            self.log_message(f"Loaded {len(template_names)} command templates")
...
def main():
    ...
    try:
        ...
        app.setApplicationName("Enhanced Network Job Runner")
