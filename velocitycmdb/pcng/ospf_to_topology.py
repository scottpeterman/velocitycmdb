...
def detect_vendor(content: str) -> str:
    if "Neighbor priority is" in content:
        return "arista"
    elif "Area" in content and "opt" in content:
        return "juniper"
    elif "Instance:" in content and "Router ID:" in content:
        return "juniper"
    elif "Router ID" in content or "ospfv3" in content.lower():
        return "arista"
    return "unknown"


def parse_neighbor_file(filepath: str) -> Tuple[str, str, List[dict]]:
...
class TopologyBuilder:
    ...
    def _same_subnet(self, ip1: str, ip2: str) -> bool:
        ...
        Permissive: same /24 subnet is sufficient.
        """
        ...
        for link in self.links:
            ...
            if remote_host not in schema[local]["peers"]:
                schema[local]["peers"][remote_host] = {
                    "ip": link.remote_router_id if is_ip_address(link.remote_router_id) else self.host_to_router_id.get(remote_host, ''),
                    "platform": self.infer_platform(remote_host),
                    "connections": []
                }
