...
class LLDPDetailCollector:
    ...
    def collect_device_lldp_detail(self, device: Dict) -> Dict[str, any]:
        ...
        try:
            ...
            # Stage 2: Collect LLDP detail for each interface (SAME connection, shell mode)
    ...
    def _collect_interface_lldp(self, host: str, port: int, device_name: str,
        ...
        Collection now happens in a single SSH session per device.
        """
        raise NotImplementedError("This method has been replaced by single-session collection")
