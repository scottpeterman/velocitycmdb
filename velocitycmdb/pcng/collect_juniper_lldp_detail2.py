...
def extract_device_name_from_prompt(prompt: str) -> Optional[str]:
    """
    Extract device name from Juniper prompt.

    Examples:
        'admin@edge1.iad1>' -> 'edge1.iad1'
        'admin@edge1.iad1#' -> 'edge1.iad1'
        'root@qfx5100-spine1>' -> 'qfx5100-spine1'
        '{master:0}[edit]' -> None (config mode, no device name)
        'edge1.iad1>' -> 'edge1.iad1'
    """
    if not prompt:
        return None
    ...
    # Pattern 1: user@device> or user@device#
    match = re.match(r'^[a-zA-Z0-9_-]+@([a-zA-Z0-9._-]+)[>#$%]$', prompt)
    if match:
        return match.group(1)

    # Pattern 2: device> or device# (no username)
    match = re.match(r'^([a-zA-Z0-9._-]+)[>#$%]$', prompt)
    if match:
        return match.group(1)

    # Pattern 3: Handle {master:X} prefix
    match = re.match(r'^\{master:\d+\}\s*[a-zA-Z0-9_-]+@([a-zA-Z0-9._-]+)[>#$%]$', prompt)
    if match:
        return match.group(1)
...
class LLDPDetailCollector:
    ...
    def _create_ssh_client(self, host: str, port: int = 22) -> SSHClient:
        ...
        options = SSHClientOptions(
            ...
            invoke_shell=True,  # MUST use invoke_shell
            timeout=120,
            shell_timeout=10,
            inter_command_time=0.5,
            prompt_count=1,  # We expect 1 prompt after each command
            debug=self.debug
        )

        # Suppress output callback for cleaner logs unless debug
        if not self.debug:
    ...
    def collect_device_lldp_detail(self, device: Dict) -> Dict[str, any]:
        ...
        try:
            ...
            self._log(f"  [{device_name}] Connecting...")
            ssh_client = self._create_ssh_client(host, port)
            ssh_client.connect()

            # CRITICAL: Use find_prompt() to detect the actual prompt
            self._log(f"  [{device_name}] Detecting prompt...")
            detected_prompt = ssh_client.find_prompt()

            if not detected_prompt:
                raise RuntimeError("Could not detect device prompt")

            self._log(f"  [{device_name}] Detected prompt: '{detected_prompt}'")

            # Extract actual device name from prompt if we can
            prompt_device_name = extract_device_name_from_prompt(detected_prompt)
            if prompt_device_name:
                self._log(f"  [{device_name}] Device name from prompt: '{prompt_device_name}'")
                # Use the prompt-derived name for the output file
                device_name = prompt_device_name
                result['device'] = device_name

            # Set the expect_prompt so execute_command knows when output is complete
            ...
            intf_output = ssh_client.execute_command('show interfaces terse | no-more,')
            ...
            for i, intf in enumerate(lldp_interfaces):
                ...
                try:
                    ...
                    cmd = f'show lldp neighbors interface {intf} | no-more,'
                    intf_output = ssh_client.execute_command(cmd)

                    # Check if there's actually a neighbor
                    if intf_output and 'LLDP Neighbor Information:' in intf_output:
                        # Clean the output - remove the command echo and prompt
                        clean_output = self._clean_command_output(intf_output, cmd, detected_prompt)
                        aggregated_output.append(f"--- Interface: {intf} ---")
                        aggregated_output.append(clean_output)
                        aggregated_output.append("")
                        neighbor_count += 1
                        if self.verbose:
                            self._log(f"    [{device_name}] Found LLDP neighbor on {intf}")
            ...
            # Stage 3: Save aggregated output with final cleanup
            output_file = self.output_base_dir / f"{device_name}.txt"
            raw_output = '\n'.join(aggregated_output)
            clean_output = self._final_cleanup(raw_output)
            self._log(f"  [{device_name}] Final cleanup: {len(raw_output)} -> {len(clean_output)} chars")
    ...
    def _clean_command_output(self, output: str, command: str, prompt: str) -> str:
        """
        Clean command output by removing command echo and trailing prompt.
        """
        lines = output.split('\n')
        clean_lines = []

        # Skip lines that contain the command we sent
        cmd_base = command.split('|')[0].strip()  # Get base command without pipes
        ...
        for line in lines:
            ...
            if cmd_base in line_stripped:
                continue

            # Skip lines that are just the prompt
            if line_stripped == prompt or line_stripped.endswith(prompt):
                continue

            # Skip any line containing {master}
            if '{master' in line_stripped:
                continue
    ...
    def _final_cleanup(self, output: str) -> str:
        """
        Final cleanup pass on entire output before writing to file.
        Removes any remaining {master} lines and cleans up excessive blank lines.
        """
        lines = output.split('\n')
        clean_lines = []
        prev_blank = False
        ...
        for line in lines:
            ...
            # Skip any line that contains {master} anywhere
            # This catches: {master}, {master:0}, {master:1}, etc.
            if '{master' in line_stripped:
                continue

            # Track blank lines to avoid multiple consecutive blanks
    ...
    def collect_from_devices(self, devices: List[Dict]) -> Dict[str, any]:
        ...
        # Collect from devices in parallel (limit concurrency)
        max_parallel = min(5, len(devices))
...
Examples:
  ...
  python collect_juniper_lldp_detail.py sessions.yaml -u admin --use-keys

  # Collect from specific device
  python collect_juniper_lldp_detail.py sessions.yaml -u admin --use-keys --name "qfx.iad1"

  # Debug mode with verbose output
  python collect_juniper_lldp_detail.py sessions.yaml -u admin --use-keys -v -d

  # Custom output directory
  python collect_juniper_lldp_detail.py sessions.yaml -u admin --use-keys -o /data/lldp
